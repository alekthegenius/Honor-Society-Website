...
Now, of course, you can't just give a web browser a .svelte file and expect it to read it. You have to first use SvelteKit, an app framework, to convert it into HTML and JavaScript. And so in this  section, I will show you a quick guide on how you can download the Svelte project, make your own changes, and upload the code to your web browser.
...
**Windows:**
Go to [Node.js/Download](https://nodejs.org/en/download/current) and make sure to select v22.20.0 LTS and make sure to select v22.20.0 LTS and select x64 if your computer was made in the last 10 years and your processor is 64-bit, x86 if your processor is ancient and 32-bit, or Arm 64 if you're fancy and your processor is an Arm chip. From there, you'll want to download the Windows installer and install it like any standard app. Note, you don't have to install the optional Chocolatey stuff. Before moving on, do yourself a quick favor and open up the `Command-Line` app and run the commands `node -v` and `npm -v` to verify that you installed everything correctly.
...
-- Honor-Society-Website      <- Main Folder
  ...
  -- honorsite                <- Project Folder
      ...
      -- src                      <- Folder containing all of the project's code
...
-- honors        <- Main Folder
  ...
  -- _app        <- Contains the backbone of the website
      ...
      -- Version.json  <- Contains the random identifier of the build
      -- immutables <- A collection of all of the random JavaScript files, images, and resources that is then displayed by each HTML file.
